...
public:
    ...
    // 	void *pRet = eosio::malloc( size );
	// 	// print("\nallocFromEosMemMgr: 0x");
	// 	// ::printhex( pRet, sizeof(void*) );
	// 	return pRet;
	// }
	...
	~NumberlikeArray() {
		delete [] blk; //eosio::free(blk);//swapnibble delete [] blk; //
	}
...
void NumberlikeArray<Blk>::allocate(Index c) {
	...
	if (c > cap) {
		...
		delete [] blk; //eosio::free(blk);//swapnibble 
		// Allocate the new array
		cap = c;
		blk = new Blk[cap]; //(Blk *)eosio::malloc( sizeof(Blk)*cap);//swapnibble new Blk[cap]; //
	}
}
...
void NumberlikeArray<Blk>::allocateAndCopy(Index c) {
	...
	if (c > cap) {
		...
		delete [] oldBlk; //eosio::free(oldBlk);//swapnibble delete [] oldBlk; //
	}
}
